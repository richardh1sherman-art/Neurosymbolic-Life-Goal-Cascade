...
class ForestSituationInferencePipeline:
    ...
    def evaluate_forest_logic(self, node, feature_vector):
        ...
        if node.is_leaf:
            # 🚨 FIX 1: Cleanly unwrap array containers to extract the raw string classification
            raw_class = node.classification
            if isinstance(raw_class, list):
                raw_class = raw_class[0] if raw_class else "unknown"
            # 🚨 FIX 2: Standardize to lowercase to prevent Prolog variable interpolation faults
            return str(raw_class).lower().strip()
    ...
    def run_forest_cascade(self):
        ...
        for story in multi_sentence_stories:
            ...
            print(f"📥 Situation ID: [{story['id']}] ──➔ Clean Atom Leaf: **schema_{inferred_schema}**")
        ...
        print("-" * 95)
        print("💾 [FS UPDATE]: Situation-level atomic facts cleanly synchronized.")
        print("=" * 95 + "\n")
